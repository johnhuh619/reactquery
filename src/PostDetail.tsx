...
async function updataePost(postId:number) {
  const response = await fetch(
    `https://jsonplaceholder.typicode.com/postId=${postId}`,
    {
      method: 'PATCH',
      body: JSON.stringify({ title: "react query 업데이트 입니다!"})
    }
  );
  return response.json();
}
export function PostDetail({ post }: IPostDetailProps) {
 ...
 const updateMutation = useMutation((postId :number) => updataePost(postId));
  return (
    <>
